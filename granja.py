            ...
            for animal, cantidad in data.inventario.items():
                if cantidad > 0 and animal in PRODUCCION_POR_SEGUNDO:
                    ...
                    for _ in range(cantidad):
                        produccion_individual = random.randint(minimo, maximo)  # ✅ Variable temporal
                        total_producido += produccion_individual
            
            if total_producido > 0:
                # Actualizar saldo global SUMANDO la producción
                data.saldo_user += total_producido  # ✅ Acumular, no sobrescribir
                ...
                if granja_estado["segundos_totales"] % 5 == 0:
                    print(f"💰 [GRANJA] {animales_activos} animales generaron {total_producido} G")
                    print(f"🏦 Saldo actual: {data.saldo_user} G")  # ✅ Usar data.saldo_user
...
def iniciar_granja():
    ...
    try:
        ...
        if granja_estado["activo"]:
            print(f"{Fore.CYAN}La granja ya está activa")
            return
        ...
        if total_animales == 0:
            print(f"{Fore.RED}No tienes animales en tu inventario.")
...
""")
        ...
        print(f"{Fore.RED}Error al iniciar granja: {e}")

def detener_granja():
    """Detiene el sistema de producción automática"""
    if not granja_estado["activo"]:
        print(f"{Fore.RED}La granja no está activa")
        return
...
""")
            ...
            if opcion == "1":
                ...
                restart()
                print("❌ Opción inválida. Elige una opción del 1 al 4.")
    except KeyboardInterrupt:
        print("Menú interrumpido")
        detener_granja()
    except Exception as e:
        print(f"Error en el menú de granja: {e}")
...
if __name__ == "__main__":
    print("SISTEMA DE GRANJA - MODO PRUEBA")
    print("=" * 55)
